            ...
            <form method="POST" action="{{ route('clients.store') }}">
                ...
                <div class="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <p class="text-sm text-blue-800">
                        <strong>Note:</strong> The client will be able to login and view their consultation history and
                        recommendations.
                    </p>
                </div>

                <div class="mb-4">
                    <label for="policy_holder_id" class="block text-gray-700 text-sm font-semibold mb-2">
                        Select Policy Holder <span class="text-red-500">*</span>
                    </label>

                    <select name="policy_holder_id" id="policy_holder_id" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('policy_holder_id') border-red-500 @enderror">
                        <option value="">-- Choose a Policy Holder --</option>

                        @foreach($policies as $policy)
                            <option value="{{ $policy->id }}" {{ old('policy_holder_id') == $policy->id ? 'selected' : '' }}>
                                {{ $policy->name }}
                            </option>
                        @endforeach
                    </select>

                    @error('policy_holder_id')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
